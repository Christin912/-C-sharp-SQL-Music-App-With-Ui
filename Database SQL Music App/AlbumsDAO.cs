     ...
     public List<JObject> getTracksUsingJoin(int albumID)
        {
            ...
            MySqlCommand command = new MySqlCommand();
            command.CommandText =
            command.CommandText = "SELECT  tracks.ID as trackID, albums.ALBUM_TITLE, `track_title`, `video_url`, " +
                "`lyrics` FROM `tracks` JOIN albums ON albums_ID = albums.ID WHERE albums_ID = @albumid";
            command.Parameters.AddWithValue("@albumid", albumID);
            command.Connection = connection;
            ...
            using (MySqlDataReader reader = command.ExecuteReader())
            {
                ...
                while (reader.Read())
                {
                    ...
                    {
                        newTrack.Add(reader.GetName(i).ToString(), reader.GetValue
                        (i).ToString());
                }
